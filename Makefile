...
# $(@F): The file-within-directory part of the file name of the target.
	...
	CGO_ENABLED=0 go build -cover -gcflags "$(GCFLAGS)" -ldflags "$(LDFLAGS)" -o $(ROOT_PATH)/$@ main.go
...
$(WEBCONSOLE)/$(GO_BIN_PATH)/$(WEBCONSOLE): $(WEBCONSOLE)/server.go $(WEBCONSOLE_GO_FILES) $(WEBCONSOLE_JS_FILES)
	...
	cp -R build ../public
	cd $(WEBCONSOLE) && \
	CGO_ENABLED=0 go build -cover -ldflags "$(WEBCONSOLE_LDFLAGS)" -o $(ROOT_PATH)/$@ ./server.go
